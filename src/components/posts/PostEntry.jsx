  ...
  const handlePostSubmit = async (formData) => {
    try {
      ...
      if (response.status === 200) {
        dispatch({
          type: actions.posts.DATA_CREATED,
          data: response.data,
        });
        onSubmit();
      ...
      <form onSubmit={handleSubmit(handlePostSubmit)}>
        ...
        <Field label="" error={errors.content}>
          <textarea
            ...
            className="h-[120px] w-full bg-gray-400 text-black p-2 rounded-md font-bold focus:outline-none lg:h-[160px]"
          ></textarea>
        </Field>
